  ...
  <div class="mx-auto px-4 max-w-6xl lg:px-8 py-2">
    <nav class="flex px-5 py-3 text-gray-700 border border-gray-200 rounded-md bg-white dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
      <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li v-for="path, pk in props.breadcrumbs" :key="path.id">
          <div class="flex items-center">
            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
            
            <p v-if="pk === props.breadcrumbs.length - 1 && route.name !== 'product-id'" class="ml-1 text-xs text-center font-medium text-gray-700 hover:text-gray-900 md:ml-2 dark:text-gray-300 dark:hover:text-gray-100">{{ path.name }}</p>
            <nuxt-link v-else :to="{ name: 'prods', query: { ct: path.id, page: 1 }}" class="ml-1 text-xs text-center font-medium text-gray-700 hover:text-gray-900 md:ml-2 dark:text-gray-300 dark:hover:text-gray-100">{{ path.name }}</nuxt-link>
